...
export default function ServicosApoioDiagnosticoTerapeutico() {
  ...
  const buildUrl = (pageNumber) => {
    ...
    return `http://localhost:8080/sadt${query ? `?${query}` : ""}`;
  };
  ...
  const handleLimpar = () => {
    limparFiltros();
    setLoading(true);
    fetch("http://localhost:8080/sadt?page=0&size=20")
      .then(res => res.json())
      .then(json => {
        setDados(json.dados || []);
        setCurrentPage(0);
      ...
      <Card>
        ...
        <Card.Header className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
          <h2 className="mb-0 text-center text-md-start">
            Serviços de Apoio e Diagnóstico Terapêutico
          </h2>
        </Card.Header>
              ...
              <Table striped bordered hover className="table-recife" responsive>
                ...
                <tbody>
                  {dados.length === 0 && !loading ? (
                    <tr>
                      <td colSpan="7" className="text-center py-4">Nenhum serviço SADT encontrado.</td>
                    </tr>
                  ) : loading && dados.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="text-center py-4"><Spinner animation="border" variant="primary" /></td>
                    </tr>
                  ) : dados.map((unidade, index) => (
                    <tr key={index}>
                      <td>{unidade.nomeOficial}</td>
                      <td>{unidade.bairro}</td>
                      <td>{unidade.tipoServico}</td>
                      <td>{unidade.comoUsar}</td>
