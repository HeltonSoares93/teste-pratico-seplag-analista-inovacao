...
export default function UnidadesDistribuicaoPreservativos() {
  ...
  const buildUrl = (pageNumber) => {
    ...
    return `http://localhost:8080/udp${query ? `?${query}` : ""}`;
  };
  ...
  const handleLimpar = () => {
    limparFiltros();
    setLoading(true);
    fetch("http://localhost:8080/udp?page=0&size=20")
      .then(res => res.json())
      .then(json => {
        setDados(json.dados || []);
        setCurrentPage(0);
              ...
              <Form>
                <Row className="g-3">
                  ...
                  <Col xs={12} md={6} lg={3}>
                    ...
                    <Form.Control placeholder="Ex: Av Doutor Eurico Chaves"
                      value={endereco}
              ...
              <Table striped bordered hover className="table-recife" responsive>
                ...
                <tbody>
                  {dados.length === 0 && !loading ? (
                    <tr>
                      <td colSpan="4" className="text-center py-4">Nenhuma UDP encontrada.</td>
                    </tr>
                  ) : loading && dados.length === 0 ? (
                    <tr>
                      <td colSpan="4" className="text-center py-4"><Spinner animation="border" variant="primary" /></td>
                    </tr>
