...
import dev.heltonsoares.backend_app.client.RecifeEducacaoInfantilEjaApiClient;
import dev.heltonsoares.backend_app.dtos.CkanEducacaoInfantilEjaResponse;
...
public class EducacaoInfantilEjaService {
  ...
  public void inicializarCache() {
    ...
    System.out.println("Iniciando cache de Educacao Infantil/EJA...");
    while (temMais) {
      try {
        ...
        System.err.println("Erro ao buscar lote do CKAN Educa̤̣o Infantil (offset " + offset + "): " + e.getMessage());
        temMais = false;
      }
    }
    System.out.println("Cache de Educa̤̣o Infantil/EJA finalizado. Total de registros: " + cache.size());
  }
  ...
  public PaginaResponse<EducacaoInfantilEjaResponse> listarDados(
    ...
    List<EducacaoInfantilEjaResponse> filtrados = cache.stream()
        ...
        .filter(ckan -> {
          ...
          if (ckan.modalidadeEnsino() == null) return false; // Nota: campo mapeado no Response ̩ modalidadeEnsino
